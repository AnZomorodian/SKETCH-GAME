...
async function startServer() {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_room', ({ roomId, playerName }) => {
      ...
      const newPlayer: Player = {
        ...
        points: 0,
        cantGuessThisRound: room.status !== 'lobby'
      };
    ...
    socket.on('refresh_game', (data) => {
      ...
      if (room && room.hostId === socket.id) {
        ...
        room.players.forEach(p => {
          p.points = 0;
          p.cantGuessThisRound = false;
        });
        io.to(roomId).emit('room_update', room);
        io.to(roomId).emit('game_refreshed');
      }
    });
    ...
    socket.on('change_host', ({ roomId, newHostId }) => {
      const room = rooms.get(roomId);
      if (room && room.hostId === socket.id) {
        const hasPlayer = room.players.some(p => p.id === newHostId);
        if (hasPlayer) {
    ...
    socket.on('guess', ({ roomId, text }) => {
      ...
      if (room && room.status === 'playing' && socket.id !== room.currentDrawerId) {
        ...
        if (!player || player.cantGuessThisRound) return;
  ...
  function startNewRound(roomId: string) {
    ...
    // Clear late-joiner guess restrictions for all players
    room.players.forEach(p => p.cantGuessThisRound = false);
